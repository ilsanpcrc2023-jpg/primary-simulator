...
**시뮬레이터 버전**: **v7.7.6** (v7.5 데이터 + v7.6.x~v7.7.x 산식·UI 보완 · 최신: PF Track 배수 A 0 / B 0.5 / C 1.0)
...
10. **⚙️ 데이터 관리** (접힘)
    ...
    - 테이블 컬럼 (v7.5.12 · v7.7.2 F 표시 전용): `환자군 | A(=T/NT) | CR | B(=A×CR) | C1(=1−L1) | PB(=B×C1) | F(기능보정율 % · 표시 전용, PF 슬라이더로만 변경) | PF(=B×F) | P(=PB+PF) | NT(전체 환자수) | RN(일만시 환자수) | 분포비(% · 기준 = 등록, 표시 전용) | 본인부담비(% · 디폴트 26.1% · 공단 지출: 참여 전 총 외래비 × (1−본인부담비), 참여 후 PB·타원비·비등록 × (1−본인부담비) · PF·성과 제외, v7.6.3~v7.6.7)` — 분포비 변경은 프리셋 버튼으로만
...
**내보내기** (v7.7.2): `handleExport`가 수기 입력·계산 사용 열만 **7열** `환자군 | A | CR | C1(%) | NT | RN | 본인부담비(%)`로 출력 (F 보정율은 v7.7.2부터 테이블 표시 전용이라 제외). 내보낸 파일을 그대로 업로드하면 테이블이 동일하게 복원(L = 1 − C1, 분포비 = RN 비율, M1 보존, PF는 B 변경 시 비율 보존). (v6.4 4열 템플릿·NHIS 엑셀도 업로드 호환 유지.)
...
pt_base·baseN_per_clinic·M_clinics·regDist(분포비 프리셋)·PF(F_g, PF 슬라이더 전용)는 엑셀로 이관 안 됨 (UI에서만 설정). v7.7.2부터 A·CR·C1·NT·RN·본인부담비 6열만 내보내기·업로드 라운드트립. B·PB·PF·P는 엑셀에 없고 업로드 시 A × CR 등으로 재산출(자가오염 방지).
