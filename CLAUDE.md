...
**시뮬레이터 버전**: **v6.9.2** (정책 모드 — F 균형추 보정 모듈 신설 · v6.9.1 누적)
  · **F 균형추 보정 모듈 (정책 모드 전용 · F 박스 직후 마운트)** — 추 위치(0~10%)로 "현재 공단 외래 지출(`T.nhi`) 대비 추가 투입 강도"를 정하면 ΔF_total = T.nhi × pct/100을 환자군별로 자동 분배해 4군 F 슬라이더 갱신. 의원 모드 가드 (`mode === "policy"` 체크 — 의원 모드에서는 미노출).
  · **분배 규칙 3종** — `distribute(deltaTotal, rule, B, n_total_g)` 함수에서: ① **HCC 비례** (B_g × n_reg 가중, 위험도 高 → 두텁게), ② **균등** (1인당 동일 ΔF), ③ **역비례** (1/B 가중, 경증 등록 인센티브). 모든 규칙에서 `Σ ΔF_g × n_reg_g = deltaTotal` 합산 보존.
  · **신호등 임계값** — 0~2% 🟢 재정중립 / 2~5% 🟡 적극 투자 / 5~8% 🟠 고투자 / 8% 초과 🔴 협상 한계. **시각적 가이드 (정책 근거 없음)** — 향후 사용자 질의 시 "이상현 교수 결정, 협상 가이드 메타포"로 답변.
  · **윈윈 카드 좌·우 (Shared Saving 분리 원칙)** — 좌 🟢 **포괄관리 성과가산 잠재** (`Σ 0.05 × B × n_reg × trackMul / M`, L2 5%p 추가 개선 가정 시 의원당 추가 가산 · 보조 표시). 우 🔵 **의원 수입 강화 (F 가산)** (`Σ ΔF[i] × regDist[i]`, 의원당 즉시 효과). **Shared Saving은 본 모듈에 일체 포함하지 않음** (별도 풀, 일차의료수가 아님 — 사용자 명시).
  ...
  · **AI 산출 결과 박스** — 인디고 그라디언트 + 펄스 닷. 환자군별 F_new(현재 F + ΔF)와 ΔF 절대값 동시 표시. "✓ 위 F 값을 슬라이더에 적용" 버튼 클릭 시 `setFAll(F_new)` + 직전 F 백업(`appliedSnapshot`). 적용 후 "↩ 직전 F로 되돌리기" 버튼 노출 (백업이 있으면).
  ...
  · **도메인 매핑 명시** (메모 박스에 표시): 분모 = `T.nhi` (현 시뮬·L2 반영), 분자 = 사업 참여 등록환자 합계(`Σ G[i].n_reg`).
...
│   ├── WinWinWin.jsx
│   ├── FBalanceCorrection.jsx   # v6.9.2: F 균형추 보정 (정책 모드 전용 · distribute·signalLevel export)
│   └── shared/NumBox.jsx
├── data/presets/
│   ├── index.js
