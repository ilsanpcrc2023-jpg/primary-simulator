...
**시뮬레이터 버전**: **v6.8.3** (TCard 의원 모드 라벨 정정 "환자군별 공단지급 수가 → 일차의료수가" + 카드 그리드 하단 안내 문구 "💡 의원 수입 = 일차의료수가(공단지급) + 환자 본인부담(현행 외래비의 30%)" 추가; v6.8.2 누적)
