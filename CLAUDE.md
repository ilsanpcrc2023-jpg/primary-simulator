...
- 등록환자 (환자군 모형): `(PB_g + F_g + PB_g × 본인부담비) × n_reg_g` (PB_g = B × (1 − L1_g), 본인부담비 디폴트 30%) — **v7.6.0: M1 → PB**
...
② 모형 효과 = Σ n_reg_g × (ab_reg_new_g − PB_g)     (= n_reg × (PF + PB × 본인부담비) · v7.6.0: M1 → PB)
...
9. **📐 수가 산출 구조 (v6.7 L1·L2 분리)** (맨 아래 접힘)
   - `P_g = B_g × (1 − L1_g) + F_g` / `공단지급 = P` / `본인부담 = PB × 본인부담비` (v7.6.0: M1 → PB)
   - `성과급_L2 = Σ max(0, L1_g − L2) × B_g × n_reg_g × TrackMul` (의원 100% 환원, 공유율 없음)
10. **⚙️ 데이터 관리** (접힘)
    - 엑셀 업로드/내보내기, 파일럿 로드, 공식 baseline 등록, 환자군별 상세 편집 테이블
    - 테이블 컬럼 (v7.5.12): `환자군 | A(=T/NT) | CR | B(=A×CR) | C1(=1−L1) | PB(=B×C1) | F(기능보정율 %) | PF(=B×F) | P(=PB+PF) | NT(전체 환자수) | RN(일만시 환자수) | 분포비(% · 기준 = 등록, 표시 전용) | 본인부담비(%)` — 분포비 변경은 프리셋 버튼으로만
...
6. **기호 B의 이중 역할 주의**: 본인부담(PB×본인부담비, v7.6.0 이전 M1×30%)은 기호 없이 "본인부담" 텍스트만 사용.
