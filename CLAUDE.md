...
| 본인부담비 | 환자군별 본인부담비 (디폴트 **26.1%** · v7.6.4 · 테이블 편집 가능). **v7.6.3: 참여 전 공단 지출 baseline `nhi0 = C1 × N × (1 − 본인부담비)`에만 사용**, 참여 후 산식에는 없음 (v7.6.1) | 0~1 |
...
10. **⚙️ 데이터 관리** (접힘)
    ...
    - 테이블 컬럼 (v7.5.12): `환자군 | A(=T/NT) | CR | B(=A×CR) | C1(=1−L1) | PB(=B×C1) | F(기능보정율 %) | PF(=B×F) | P(=PB+PF) | NT(전체 환자수) | RN(일만시 환자수) | 분포비(% · 기준 = 등록, 표시 전용) | 본인부담비(% · 디폴트 26.1% · 참여 전 공단 지출 × (1−본인부담비), v7.6.3/v7.6.4)` — 분포비 변경은 프리셋 버튼으로만
...
7. **참여 전 공단 지출 = 총 외래비 × (1 − 본인부담비) (v7.6.3)**: baseline `nhi0`에만 본인부담비를 적용한다. 참여 후 `nhi`에는 적용하지 않는다.
