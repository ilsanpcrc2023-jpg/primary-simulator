...
| regDist | 의원당 환자군별 등록환자수 배열 (v7.5.4: 0.1명 단위, 디폴트 = NT 기준 ratio_i × 1,000 [287.8, 211.0, 251.2, 250.0], 합 1,000 · 등록 분포비 % = RR/10) | 명 |
...
10. **⚙️ 데이터 관리** (접힘)
    ...
    - 테이블 컬럼 (v7.5.4): `환자군 | A(=T/NT) | CR | B(=A×CR) | C1(=1−L1) | PB(=B×C1) | F(기능보정율 %) | PF(=B×F) | P(=PB+PF) | 본인부담비(%) | NT(전체 환자수) | RN(일만시 환자수) | 기준 분포비(% · NT 기준) | 등록 분포비(% · RR/1000)`
