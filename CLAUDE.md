...
**시뮬레이터 버전**: **v6.9.2** (정책 모드 — F 균형추 보정 모듈 신설 · v6.9.1 누적)
  ...
  · **추 → ΔF 자동 분배** — 추 위치(0~10%)로 "현재 공단 외래 지출(`T.nhi`) 대비 추가 투입 강도"를 정하면 ΔF_total = T.nhi × pct/100을 환자군별로 자동 분배해 4군 F 슬라이더 갱신. 분배 규칙 3종: ① **HCC 비례** (B × n_reg 가중, 위험도 高 → 두텁게), ② **균등** (1인당 동일 ΔF), ③ **역비례** (1/B 가중, 경증 등록 인센티브). 모든 규칙에서 `Σ ΔF_g × n_reg_g = deltaTotal` 합산 보존.
