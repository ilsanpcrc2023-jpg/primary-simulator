...
**시뮬레이터 버전**: **v6.10.0** (PF 단순화 — 통합 슬라이더 + 분배 토글, 균형추 모듈 폐지 · 정책 시나리오 프리셋 신설)
