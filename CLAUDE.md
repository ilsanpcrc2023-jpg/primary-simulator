...
**시뮬레이터 버전**: **v7.5** (의료비 0원 제외 baseline 적용 — 데이터만, 산식은 v7.2.3 유지)
  ...
  · **v7.5.6 — NT·RN 열 삭제 + 기준 분포비 수기값 엔진 적용 (2026-09-04, 사용자 결정)** — UI·state만.
    - 상세 편집 테이블에서 NT·RN 열 제거 (v7.5.4에서 추가했던 것). 컬럼 11개로 복귀: `A | CR | B | C1 | PB | F | PF | P | 본인부담비 | 기준 분포비 | 등록 분포비`.
    - **기준 분포비 수기 입력값이 엔진에 적용** — `ratios` 메모 = `baseRatios ?? N_i/ΣN` (v7.5.3 방식 복원). 수기값이 N_g·baseN_g·ffsPerPerson에 그대로 반영되어 KPI가 움직임. 디폴트(override 없음) = RN 실측 = **20.16 / 19.77 / 29.38 / 30.68 %**. 합 100% 강제 없음.
    - 단위 테스트 94/94 통과.
...
10. **⚙️ 데이터 관리** (접힘)
    ...
    - 테이블 컬럼 (v7.5.6): `환자군 | A(=T/NT) | CR | B(=A×CR) | C1(=1−L1) | PB(=B×C1) | F(기능보정율 %) | PF(=B×F) | P(=PB+PF) | 본인부담비(%) | 기준 분포비(% · 수기 적용, 디폴트 RN 실측) | 등록 분포비(% · RR/1000)`
