...
**시뮬레이터 버전**: **v7.9.0** (v7.9.0 데이터 260909 + v7.6.x~v7.8.x 산식·UI 보완 · 최신: v7.9.0 엑셀 260909 업로드값을 공식 baseline 디폴트로 [A·CR·C1·RN·군별 본인부담비], v7.8.1 C 상한 +25%p 복귀, v7.8.0 PF 상한 40%, v7.7.9 테이블 캡션 문구, v7.7.8 풋노트 미표시, v7.7.7 NT 열 미표시·엑셀 제외, v7.7.6 PF Track 배수 A 0 / B 0.5 / C 1.0)
  ...
  · **v7.5.12 — 본인부담비 열을 맨 오른쪽으로 이동 (2026-09-04, 사용자 결정)** — UI만. 컬럼 13개: `A | CR | B | C1 | PB | F | PF | P | NT | RN | 분포비(표시 전용) | 본인부담비`.

  · **v7.6.0 — M1 폐기: 모든 산식에서 M1 → PB(= B × (1−L1)) 대체 (2026-09-04, 사용자 결정 · 산식 변경)**
    - 사용자: "전체적으로 M1 값은 PB값으로 대체. M1 값은 사용하지 않을 거야." → 엔진 `G`에 `PB_g = B × (1−L1_g)`를 두고 M1이 쓰이던 모든 자리를 치환.
    - 치환 목록: 본인부담 `PB × 본인부담비` · baseline FFS `inc0 = PB × N` · 비등록 FFS `PB × n_unreg` · 공단 외래비 `C1 = PB/(1−L)`, `D1_L2 = PB × L2/(1−L2)` · Track A `tA = PB + F` · `ffsPerPerson = Σ ratio × PB` · 패널효과 `Σ PB × (N − baseN)` · 모형효과 `Σ n_reg × (ab_reg − PB)` (= n_reg × (PF + PB×본인부담비)) · T 비등록 합산 · FCard `pfBaseline = Σ regDist × PB × M`.
    - `base.M1`은 데이터 필드(엑셀 파서·baseline·export)로만 보존, 계산에서 완전 제외. 상세 편집 테이블 본인부담비 부제 "PB 대비".
    - KPI 디폴트 변화(의원 100개·Track C): 의원 수입 변화 +5,768만원 → **+8,377만원**(의원당), 공단 지출 변화 +48.7억 → **+75.8억원**. 이유: 모형효과가 `n_reg × (ab_reg − M1) = n_reg × (PF + PB×0.3 − 0.7×M1)`에서 `n_reg × (PF + PB×0.3)`으로 바뀌어, PB < M1이던 4군 등에서 음(−)으로 작용하던 `PB − M1` 항이 사라짐. 단위 테스트 95/95 (M1 기대값 → PB로 갱신).

  · **v7.6.1 — 본인부담 항 완전 제거 (2026-09-04, 사용자 결정 · 산식 변경)** — "참여 후에 M1×0.3이 더해지는데 잘못 추가된 값, 다 없애줘".
    - 엔진: `ab_reg = pay_gov = PB + PF` (이전 `+ PB × 본인부담비`). 의원 수입(inc·T.inc·tC·tB), 공단 지출(nhi), 모형효과(= n_reg × PF) 모두에서 본인부담 항 제거. `G.B`(본인부담 표시값) 삭제.
    - state `copayRates` · reducer `SET_COPAY_AT` · `updCopay` · constants `COPAY_RATE`/`INIT_COPAY_RATES` · 테이블 본인부담비 열 모두 삭제. 컬럼 12개: `A | CR | B | C1 | PB | F | PF | P | NT | RN | 분포비`.
    - KPI 디폴트(의원 100개·Track C): 의원 수입 변화 +8,377만원 → **+3,537만원**(= 의원당 PF 가산 Σ n_reg × PF 3,176만원 + 포괄관리성과 361만원, 패널효과 ≈ 0), 공단 지출 변화 +75.8억 → **+27.4억원**. 단위 테스트 93/93 (COPAY 테스트 2개 삭제).

  · **v7.6.2 — 본인부담비 열 복원 (참고 항목, 2026-09-04, 사용자 지적)** — UI·state만, 산식 불변.
    - v7.6.1에서 본인부담 항을 산식에서 빼면서 열까지 삭제한 것을 되돌림. 맨 오른쪽에 **본인부담비(% · 참고, 산식 미반영)** 열 복원 — 편집·보존(`state.copayRates`, `SET_COPAY_AT`, `updCopay`, `COPAY_RATE`/`INIT_COPAY_RATES`)만 하고 의원 수입·공단 지출 어디에도 쓰지 않음. 컬럼 13개: `A | CR | B | C1 | PB | F | PF | P | NT | RN | 분포비 | 본인부담비(참고)`. 단위 테스트 94/94.

  · **v7.6.3 — 참여 전 공단 지출 × (1 − 본인부담비) (2026-09-04, 사용자 결정 · 산식 변경)** — "참여전 공단 지출에 본인부담비를 곱하기" (선택지 확인: × (1 − 본인부담비)).
    - 엔진 `G`: `nhi0 = C1 × N × (1 − copayRates[i])` (참여 전 = FFS 총 외래비 중 공단 부담분). 참여 후 `nhi = (P + D1_L2) × n_reg + C1 × n_unreg`는 불변 (v7.6.1 본인부담 항 없음). 의원 수입(inc0·inc·decomp)도 불변.
    - `copayRates`를 엔진 메모 의존성에 추가 → 상세 편집 테이블 본인부담비 편집이 공단 지출 KPI에 즉시 반영 (1군 0%로 바꾸면 +836.7억 → +775.4억). 헤더 부제 "% · 참고 (산식 미반영)" → "% · 참여 전 공단 지출", 공단 지출 카드 부제에 "참여 전 = 총 외래비 × (1 − 본인부담비)" 명시.
    - KPI 디폴트(의원 100개·Track C): 공단 지출 변화 +27.4억 → **+836.7억원** (baseline이 총 외래비 ≈2,698억의 70%로 축소된 만큼 증가), 의원 수입 변화 **+3,537만원** 불변. 단위 테스트 +1 (95/95 통과).

  · **v7.6.4 — 본인부담비 디폴트 30% → 26.1% (2026-09-04, 사용자 결정)** — 상수만.
    - `COPAY_RATE = 0.261`, `INIT_COPAY_RATES` 4군 공통. 테이블 본인부담비 열 디폴트 26.1%, 참여 전 공단 지출 baseline = 총 외래비 × 73.9%.
    - KPI 디폴트(의원 100개·Track C): 공단 지출 변화 +836.7억 → **+731.5억원**, 의원 수입 변화 **+3,537만원** 불변. 단위 테스트 95/95 (COPAY 기댓값 갱신).

  · **v7.6.5 — 참여 후 공단 지출: 등록환자 PB × (1 − 본인부담비) + PF (2026-09-04, 사용자 결정 · 산식 변경)** — "참여 후 공단지출에서 PB에는 (1−환자부담금)을 곱해주고, PF는 그대로 적용".
    - 엔진 `G`: `nhi_reg = PB × (1 − copayRates[i]) + PF` 신설, `nhi = (nhi_reg + D1_L2) × n_reg + C1 × n_unreg`. PF는 전액 공단 부담. **의원 수입 `ab_reg = P = PB + PF`는 불변** — 차액 PB × 본인부담비는 환자 본인부담. 타원비 `D1_L2`·비등록 `C1 × n_unreg` 항은 지시 범위 밖이라 그대로(100%).
    - `G[i].nhi_reg`·`G[i].copay` 반환값 추가. 공단 지출 카드 부제·본인부담비 헤더("% · 공단 지출 (PB 부분)")·풋노트 갱신.
    - KPI 디폴트(의원 100개·Track C): 공단 지출 변화 +731.5억 → **+689.4억원** (Σ PB × n_reg ≈ 161.3억의 26.1% = 42.1억 감소), 의원 수입 변화 **+3,537만원** 불변. 1군 본인부담비 0%: +639.6억. 단위 테스트 +1 (96/96 통과).

  · **v7.6.6 — 참여 후 공단 지출: 타원비·비등록·포괄관리성과에도 (1 − 본인부담비) (2026-09-04, 사용자 결정 · 산식 변경)** — "D1_L2·비등록 C1·포괄관리성과에 (1 − 본인부담비) 곱해주기".
    - 엔진 `G`: `nhi = (PB × (1−cp) + PF + D1_L2 × (1−cp)) × n_reg + C1 × (1−cp) × n_unreg`. `performance.perf_nhi = Σ max(0, L1_g − L2) × B_g × n_reg_g × (1 − cp_g) × TrackMul` 신설 → `nhiTotal = T.nhi + perf_nhi` (KPI 카드·WinWinWin 텍스트 동일). **의원 수입 `incTotal = T.inc + perf_blended`·decomp는 불변** (성과는 의원 100% 수령, 공단 부담분만 73.9%).
    - 결과: 참여 전·후 본인부담비 적용 범위가 대칭이 되어 공단 지출 변화 = PF 가산 + 성과 공단분 + L2(가중평균) vs L1 타원비 차이만 남음. PF만 전액 공단 부담.
    - 본인부담비 헤더 부제 "% · 공단 지출 (PF 제외)", 공단 지출 카드 부제·풋노트 갱신.
    - KPI 디폴트(의원 100개·Track C): 공단 지출 변화 +689.4억 → **+28.5억원** (= PF 31.8억 + 성과 2.7억 − 타원비 L2 효과 5.9억), 의원 수입 변화 **+3,537만원** 불변. 1군 본인부담비 0%: +29.8억. 단위 테스트 +1 (97/97 통과).

  · **v7.6.7 — 포괄관리성과 공단 지출 100% 복귀 (2026-09-04, 제안 → 사용자 승인 · 산식 변경)** — 성과급은 진료비 청구가 아니라 공단→의원 직접 지급이라 환자 본인부담이 없음. v7.6.6에서 성과에 곱했던 (1 − 본인부담비)를 성과 항목만 되돌려 **공단 지출 성과 = 의원 수령액(perf_blended)** 으로 정합. 타원비 D1_L2·비등록 C1의 (1 − 본인부담비)는 유지.
    - 엔진 `performance.perf_nhi = perf_blended`. `nhiTotal = T.nhi + perf_blended`. 본인부담비 헤더 부제 "% · 공단 지출 (PF·성과 제외)", 공단 지출 카드 부제 "참여 후 = (PB + 타원비 + 비등록) × (1 − 본인부담비) + PF + 포괄관리성과".
    - KPI 디폴트(의원 100개·Track C): 공단 지출 변화 +28.5억 → **+29.5억원** (성과 공단분 2.67억 → 3.61억), 의원 수입 변화 **+3,537만원** 불변. 단위 테스트 +1 (98/98 통과).

  · **v7.6.8 — 군별 L2_g = L1_g − Δ (2026-09-04, 제안 → 사용자 승인 · 산식 변경)** — L2를 4군 가중평균 스칼라 하나로 전 군에 적용하던 구조에서, 디폴트(ΔC = 0)인데도 군별 L1과 평균의 차이로 타원비 −5.9억·성과 3.6억이 발생하던 문제 해소.
    - 엔진: `dL2 = L1avg − L2eff` (C 슬라이더의 ΔC), `L2_g = clamp(L1_g − dL2, 0, 0.999)`. G의 타원비 `D1_L2 = PB × L2_g/(1 − L2_g)`, 성과 `max(0, L1_g − L2_g)` 모두 군별 L2_g 사용 → Δ = 0에서 참여 전·후 타원비 동일, 성과 0. `performance`에 `dL2`·`L2_g` 반환 추가. `state.L2`(평균 수준 목표)·`setL2`·C 슬라이더 UI(C0 = 1 − L1avg, Cnow = 1 − L2_display)는 그대로.
    - KPI 디폴트(의원 100개·Track C): 의원 수입 변화 +3,537만원 → **+3,177만원**(= PF만), 공단 지출 변화 +29.5억 → **+31.8억원**(= PF만). C +5%p: 의원 +6,353만원 · 공단 −13.8억, C +10%p: 의원 +9,530만원 · 공단 −37.5억. 단위 테스트 +2 (100/100 통과).

  · **v7.6.9 — 상세 편집 테이블 한 화면 폭 맞춤 (2026-09-04, 사용자 결정)** — UI만, 산식·state 불변.
    - `minWidth: 1140` 제거(카드 폭 max-w-4xl ≈ 838px 안에서 가로 스크롤 없이 13열 모두 표시). 헤더 부제 축약(`=T/NT`·`=A×CR`·`=1−L1 %`·`=B×C1`·`보정율 %`·`=B×F`·`=PB+PF`·`전체`·`일만시`·`%`·`% · 공단`) — 전체 설명은 `title` 툴팁 유지. 입력 폭 A 80→64px, CR 64→46px, C1·F·본인부담 64→44px, NT·RN 96→70px, 셀 `px-1`, DraftInput `px-0.5`. "본인부담비" 헤더는 "본인부담"으로 축약. `overflow-x-auto`는 좁은 화면 fallback으로 유지.
    - Playwright 측정(1300·1024px 뷰포트): 테이블 838px = 래퍼 838px, overflow 없음. 단위 테스트 100/100.

  · **v7.7.0 — 엑셀 내보내기·업로드에 상세 편집 테이블 수기 입력 열 반영 (2026-09-04, 사용자 결정)** — 산식 불변.
    - **내보내기** (`handleExport`): 4열(환자군·N·M1·L) → **16열** `환자군 | NT | RN | A | CR | B | C1(%) | PB | F(%) | PF | P | 분포비(%) | 본인부담비(%) | RR | M1 | L`. 수기 입력 열(NT·RN·A·CR·C1·F·본인부담비)과 호환 열(RR·M1·L)은 업로드 시 반영, 산출 열(B·PB·P·분포비)은 참고. 합계 행: NT·RN·분포비·RR SUM.
    - **업로드** (`handleFile`): `COL_ALIASES`에 `NT`·`C1`·`F`·`PF`·`COPAY` 키 신설(+ `HCC`에 "A", `CR`에 "CR" alias). 단문자 헤더는 `findColExact`(정규화 정확 일치)로만 인식 — PF↔F, N↔NT, P↔PB 부분일치 혼동 방지 (`COL_ALIASES_EXACT_ONLY`). 규칙: `C1` 있으면 `L = 1 − C1` 우선 · `F(%)` 있으면 `F_g = round(A×CR × F/100)`(테이블 규칙), 없고 `PF(원)`만 있으면 그 값, 둘 다 없으면 보존 · `본인부담비(%)` → `copayRates` (LOAD_DATA `copayRates` 옵션) · `A`·`CR`·`NT`는 `base`에 저장(없으면 이전 값 보존 → 업로드 후 테이블 A·CR 표시 유지) · `RR`은 0.1명 정밀도 보존(이전 정수 반올림 폐기).
    - **업로드 시 사업 규모 유지** (`preserveScale`): 엑셀 업로드는 `totalN`·`baseN_per_clinic`을 그대로 두고(테이블 RN 편집과 동일 규칙, RN은 환자군 배분 비율에만 반영) `datasetTotalN` anchor만 갱신. 프리셋/공식 baseline 로드는 종전대로 ΣN으로 재설정.
    - 검증: Playwright 라운드트립(1군 A·CR·C1·F·NT·RN·본인부담비, 3군 F, 4군 본인부담비 편집 → 내보내기 → 새로고침 → 업로드) 테이블 값·분포비·KPI(의원 +3,639만원·공단 +36.4억)·의원당 요약 모두 동일. 단위 테스트 +3 (103/103).

  · **v7.7.1 — 엑셀 열을 수기 입력·계산 사용 열로 한정 (2026-09-04, 사용자 결정)** — 산식 불변.
    - **내보내기 8열** `환자군 | A | CR | C1(%) | F(%) | NT | RN | 본인부담비(%)` (열 순서 사용자 지정). v7.7.0의 산출 열(B·PB·PF·P·분포비)과 파생/미사용 필드(L·M1·RR) 제거. 합계 행: NT·RN SUM.
    - **업로드**: L = 1 − C1, PF = round(A×CR × F/100), 분포비·regDist = RN 비율로 재산출(RR 없음), M1은 현재 값 보존(계산 미사용 데이터 필드). 구 템플릿(N·M1·L)·NHIS 엑셀(RR·RO 등) 인식은 호환용으로 유지.
    - 검증: Playwright 라운드트립 동일(테이블·분포비·KPI·요약). 단위 테스트 104/104.

  · **v7.7.2 — 테이블 F 보정율 표시 전용 + 엑셀에서 F 제외 (2026-09-04, 사용자 결정)** — 산식 불변.
    - 상세 편집 테이블 F(보정율 %) 셀: DraftInput → 텍스트(`F_rate.toFixed(1)%`). PF 변경은 상단 PF 통합 슬라이더·환자군별 슬라이더로만. 헤더 title "표시 전용 · 변경은 상단 PF 슬라이더", 캡션·풋노트의 편집 열 목록에서 F 제거.
    - **내보내기 7열** `환자군 | A | CR | C1(%) | NT | RN | 본인부담비(%)`. `COL_ALIASES`에서 `F`·`PF` 키 삭제(`COL_ALIASES_EXACT_ONLY` = NT·C1·COPAY). 업로드는 F(%)·PF(원) 열을 읽지 않음.
    - **업로드 시 PF 처리**: 엑셀 비반영이되, 업로드로 B(A×CR)가 바뀌면 테이블 A·CR 편집(v7.5.9)과 동일하게 `F_g = round(F_g/B_old × B_new)`로 비율 보존("B 기준 X%" 유지). B 불변이면 F_g 그대로.
    - 검증: Playwright — 행당 입력 6개(A·CR·C1·NT·RN·본인부담비), 라운드트립 테이블·PF·KPI(의원 +3,225만원·공단 +32.2억) 동일. 단위 테스트 104/104.

  · **v7.7.3 — 테이블 헤더·합계 표기 정리 (2026-09-04, 사용자 결정)** — UI만. 본인부담 열 부제 "% · 공단" → "%". 분포비 열 tfoot 합계(100.0%) 삭제(프리셋 적용 시 "프리셋" 배지만 유지). 단위 테스트 104/104.

  · **v7.7.4 — 고급 설정 사업 참여 의원 수 프리셋 10,000 추가 (2026-09-04, 사용자 결정)** — `CLINIC_COUNT_PRESETS` [100 · 1,000 · 3,000 · **10,000** · 일만시 2,923]. 선택 시 totalN = 의원당 환자수 × 10,000 (기존 SET M_clinics 규칙). 검증: 10,000 선택 시 요약 "🏥 10,000개 의원", 의원당 +3,177만원 불변 · 공단 +3,176.5억(100배). 단위 테스트 104/104.

  · **v7.7.5 — 공단 지출 변화 카드 부제 정리 (2026-09-04, 사용자 결정)** — UI만. 부제를 "사업 전체 공단 지출 변화액"만 남기고 산식 설명(참여 전/후 본인부담비 적용 문구) 삭제. 산식 설명은 상세 편집 테이블 헤더 툴팁·풋노트와 CLAUDE.md 준수사항 7에 유지. 단위 테스트 104/104.

  · **v7.7.6 — PF Track별 차등: A 0 · B 0.5 · C 1.0 (2026-09-05, 사용자 결정 · 산식 변경)** — "의원 수입에서 Track A는 PF 제외, Track B는 PF 1/2, Track C는 PF 그대로".
    - 엔진: `pfMul = clamp(hccPct/100, 0, 1)` (성과 Track 배수와 동일 보간). `G`에서 `F_eff = PF × pfMul`, `ab_reg = pay_gov = PB + F_eff`, `nhi_reg = PB × (1 − cp) + F_eff`. Track 1인당: `tA = PB` · `tB = PB + 0.5 × PF` · `tC = PB + PF` · `tS = ab_reg`. `G[i].F_per_pt = F_eff`, `F_full = PF`, `pfMul` 반환 · `performance.pfMul` 추가.
    - **공단 지출의 PF 항도 의원 수령액과 동일**(공단 직접 지급 정합, v7.6.7 성과와 같은 원칙) — 사용자 지시는 "의원 수입에서"였으나, 의원이 받지 않는 PF를 공단이 지출하는 모순을 피하기 위해 양쪽에 같은 배수 적용. 공단 지출 PF 항을 Track과 무관하게 전액으로 두려면 `nhi_reg`의 `F_eff` → `F_i` 한 줄 변경.
    - KPI 정책 모드 `pfEffect = Σ n_reg × PF × pfMul` · FCard 미니 표시 `공단지출 +X억` × pfMul (Track A/B에서 "(Track 배수 ×0.0/×0.5)" 부기). 상세 편집 테이블 풋노트 "등록환자 1인당 의원수입 = PB + PF × Track 배수".
    - **준수사항 5 "모든 Track에 F 가산" 폐기** → Track A는 PF 없음. 이전(v6.2~v7.7.5) "Track A = PB + F" 규칙은 이 결정으로 대체.
    - KPI 디폴트(의원 100개·ΔC 0): Track C **+3,177만원 / +31.8억원 불변**. Track B **+1,588만원 / +15.9억원**, Track A **+0원 / +0원**. 단위 테스트 +1 (105/105 통과).

  · **v7.7.7 — 상세 편집 테이블 NT 열 미표시 + 엑셀 NT 제외 (2026-09-05, 사용자 결정)** — UI·엑셀만, 산식 불변.
    - 테이블 컬럼 12개: `A | CR | B | C1 | PB | F | PF | P | RN | 분포비(표시 전용) | 본인부담`. 행당 입력 5개: A · CR · C1 · RN · 본인부담비. tfoot ΣNT 삭제.
    - **내보내기 6열** `환자군 | A | CR | C1(%) | RN | 본인부담비(%)` (합계 행 RN SUM). **업로드**는 NT 열을 읽지 않음 — `COL_ALIASES.NT` 삭제, `COL_ALIASES_EXACT_ONLY = ["C1","COPAY"]`. `base.NT`는 baseline 데이터 필드로만 보존(업로드 시 이전 값 유지), 계산 미사용.
    - 검증: Playwright 라운드트립(1군 A·CR·C1·RN·본인부담비, 4군 본인부담비 편집 → 내보내기 → 새로고침 → 업로드) 테이블·KPI(의원 +3,225만원·공단 +32.2억) 동일, 테이블 폭 838px 유지. 단위 테스트 105/105.

  · **v7.7.8 — 상세 편집 테이블 아래 ※ 직접 편집 풋노트 미표시 (2026-09-05, 사용자 결정)** — UI만. 테이블 하단 설명 블록(직접 편집 열 · C1/A·CR 편집 규칙 · 본인부담비 산식 · 분포비 프리셋 · RN/NT/M1 안내)을 화면에서 제거(JSX 주석으로 원문 보존). 헤더 `title` 툴팁과 CLAUDE.md 준수사항 7이 산식 설명을 대신함. 단위 테스트 105/105.

  · **v7.7.9 — 상세 편집 테이블 캡션 문구 수정 (2026-09-05, 사용자 결정)** — UI만. 헤더 캡션 `입력: A · CR · C1 · RN · 본인부담비(공단 지출)` → `입력: A · CR · C1 · RN · 본인부담비`. 단위 테스트 105/105.

  · **v7.8.0 — PF 통합 슬라이더 상한 40% · 포괄관리 지표 C 슬라이더 상한 +75%p (2026-09-05, 사용자 결정)** — UI 범위만, 산식 불변.
    - `constants.PF_PCT_MAX = 40` (이전 RegistrationPanel 로컬 상수 20) · `constants.C_DELTA_MAX = 75` (이전 TabSimulation·TabTrack 리터럴 25). 슬라이더 눈금 PF 0/10/20/30/40%, C 0/+15/+30/+45/+60/+75%p. NumBox·clamp·게이지 배경 모두 상수 참조.
    - **실효 상한**: ΔC는 C = 100%(L2 = 0)를 넘을 수 없어 `setL2`가 `L1avg − ΔC`를 0에서 clamp → 슬라이더를 75로 끌어도 약 74.5%p(= L1avg)에 멈춤. 군별 성과 diff = min(ΔC, L1_g) (3군만 L1 0.7605 > 0.75).
    - KPI(의원 100개·Track C): PF 40% → 의원 +25,412만원 · 공단 +254.1억. ΔC 50%p → 의원 +34,942만원 · 공단 **+38.0억**, ΔC 74.5%p → +50,212만원 · **+152.5억** — ΔC가 크면 공단 지출이 다시 양(+)으로 돌아섬: 타원비 절감은 (B − PB) × (1 − 본인부담비)에서 포화하는데 포괄관리성과(ΔC × B, 공단 100%)는 선형 증가하기 때문. 이동/축소 모형 불일치(미결 논점)의 연장선.
    - 단위 테스트 +2 (107/107 통과).

  · **v7.8.1 — 포괄관리 지표 C 슬라이더 상한 +25%p 복귀 (2026-09-05, 사용자 결정)** — UI 범위만. `C_DELTA_MAX` 75 → **25**, 눈금 0/+5/+10/+15/+20/+25%p (v7.8.0 이전과 동일). PF 통합 슬라이더 상한 40%는 유지. 단위 테스트 107/107.

  · **v7.9.0 — 엑셀 260909 업로드값을 공식 baseline 디폴트로 (2026-09-09, 사용자 결정 · 데이터 갱신)** — "우선은 이 엑셀의 입력값을 기본값으로". 산식 불변.
    - 원본: [docs/baseline_upload_260909.xlsx](docs/baseline_upload_260909.xlsx) (시트 `시뮬레이터_업로드`, 7열 템플릿 `환자군 | A | CR | C1(%) | NT | RN | 본인부담비(%)`). [src/data/presets/official_baseline.json](src/data/presets/official_baseline.json) version `3.1-260909` (라벨 `v3.1-260909`), dataLabel `HCC_v3.1_260909`.
    - **base 갱신**: RN=[2,291,133·2,735,995·3,335,882·3,548,115] (ΣRN 12,411,152 → **11,911,125**, 의원당 4,246 → **4,075명**, `INIT_DEFAULT_TOTAL_N` 424,600 → **407,500**) · A=[423,908·607,228·1,248,241·5,203,634] · CR=[0.5888·0.5571·0.4927·0.1862] · L=1−C1=[0.7067·0.7176·0.7305·0.7064] (C1 29.3/28.2/26.9/29.4%). M1·NT는 새 자료에 없어 v3.0 값 유지(계산 미사용·미표시). M_clinics 2,923 유지.
    - **파생값**: B = A×CR = [**249,593·338,271·615,003·968,667**] · PB=[73,203·95,542·165,743·284,439] · INIT_F = B×5% = [**12,480·16,914·30,750·48,433**] · `INIT_REG_DIST` = RN 비율 × 1,000 = [**192.4·229.7·280.1·297.9**] (합 1,000.1 → 사업 전체 등록 100,010명 · 분포비 19.24/22.97/28.01/29.79%).
    - **군별 본인부담비 디폴트 신설**: JSON `copay: [0.280506, 0.2856, 0.264511, 0.205334]` (28.1/28.6/26.5/20.5%) → `constants.INIT_COPAY_RATES`가 JSON copay(4개 0~1)에서 로드, 없으면 `[COPAY_RATE×4]`(26.1%) fallback. 상세 편집 테이블 본인부담 열 디폴트·DraftInput fallback에 반영. 공식 baseline 등록(`/api/commit-baseline`)도 `copay` 저장 (4개 숫자 0~1 검증, 소수 6자리).
    - UI 텍스트 동적화: 자료 분석 절차 2단계(환자 수·의원당 환자수 = `ON`·`INIT_PER_CLINIC`), 초기화 확인 문구 분포비 = `INIT_REG_DIST`에서 산출.
    - KPI 디폴트(의원 100개·Track C·ΔC 0): 의원 수입 변화 +3,177만원 → **+2,933만원**(= Σ regDist × PF), 공단 지출 변화 +31.8억 → **+29.3억원**. Playwright 확인: 테이블 1군 A 423,908 · CR 0.589 · C1 29.3 · PB 73,203 · PF 12,480 · P 85,683 · RN 2,291,133 · 분포비 19.2% · 본인부담 28.1. 단위 테스트 107/107 (회귀값 갱신).
...
| 본인부담비 | 환자군별 본인부담비 (디폴트 **군별 28.1 / 28.6 / 26.5 / 20.5%** · v7.9.0 baseline copay · 이전 v7.6.4 공통 26.1% = fallback `COPAY_RATE` · 테이블 편집 가능). **공단 지출에만 사용**: 참여 전 `nhi0 = C1 × N × (1 − 본인부담비)` (v7.6.3), 참여 후 PB·타원비 D1_L2·비등록 C1 × (1 − 본인부담비) (v7.6.5/v7.6.6). PF·포괄관리성과는 공단 직접 지급이라 100% (v7.6.7). 의원 수입에는 없음 (v7.6.1) | 0~1 |
| **L1** | **선지급 기준 타원이용비중 (환자군별 4개, 데이터 실측 = base.L 자동 산출 · v6.9.5)** | 0~1 |
| **L2** | **실측 타원이용비중 — v7.6.8: 군별 L2_g = L1_g − Δ (Δ = C 슬라이더 ΔC, `state.L2`는 평균 수준 목표값)** · 타원비·성과급 산정 | 0~1 |
| M1 | 1인당 현행 등록의원 외래비 — **v7.6.0부터 계산 미사용** (데이터 필드로만 보존, 모든 산식은 PB로 대체) | 원/년 |
| N | 실인원 환자수 (연인원 아님) | 명 |
| M | 사업 참여 의원 수 | 개 |
...
- 등록환자 (환자군 모형): `(PB_g + F_g × pfMul) × n_reg_g` (PB_g = B × (1 − L1_g), pfMul = Track 배수 A 0 / B 0.5 / C 1.0) — **v7.6.0: M1 → PB · v7.6.1: 본인부담 항 제거 · v7.7.6: PF Track 배수**
- 비등록환자 (FFS 유지): `PB_g × n_unreg_g` — **v7.6.0: M1 → PB**
- 의원 선지급 총수입 = 위 두 합계
...
기준 수입 (baseline) = baseN × ffsPerPerson × M     (참여 전 FFS)
  baseN = 참여 전 의원당 환자수
  ffsPerPerson = Σ ratios[i] × PB_g  (이용분포 기반 가중평균 · v7.6.0: M1 → PB)

① 패널 효과 = Σ PB_g × (N_g_after − baseN_g)        (FFS 유지 가정 · v7.6.0: M1 → PB)
② 모형 효과 = Σ n_reg_g × (ab_reg_new_g − PB_g)     (= n_reg × PF × Track 배수 · v7.7.6)
...
- Track A = PB (v7.7.6: PF 제외 · v7.6.0: FFS 기준을 M1 → PB로 대체)
- Track B = PB + 0.5 × PF (v7.7.6 · = 0.5 × Track A + 0.5 × Track C)
- Track C = PB + PF (환자군 모형 전면 · v7.6.1: 본인부담 항 없음)
- **PF는 Track 배수(A 0 / B 0.5 / C 1.0)로 차등 지급** (v7.7.6 사용자 결정 · 이전 "모든 Track에 F 가산" 원칙 대체). 포괄관리성과도 같은 배수.
...
9. **📐 수가 산출 구조 (v6.7 L1·L2 분리)** (맨 아래 접힘)
   - `P_g = B_g × (1 − L1_g) + F_g` / `공단지급 = P` / 본인부담 항 없음 (v7.6.1 제거)
   - `성과급_L2 = Σ max(0, L1_g − L2) × B_g × n_reg_g × TrackMul` (의원 100% 환원, 공유율 없음)
10. **⚙️ 데이터 관리** (접힘)
    - 엑셀 업로드/내보내기, 파일럿 로드, 공식 baseline 등록, 환자군별 상세 편집 테이블
    - 테이블 컬럼 (v7.5.12 · v7.7.2 F 표시 전용 · v7.7.7 NT 미표시): `환자군 | A(=T/NT) | CR | B(=A×CR) | C1(=1−L1) | PB(=B×C1) | F(기능보정율 % · 표시 전용, PF 슬라이더로만 변경) | PF(=B×F) | P(=PB+PF) | RN(일만시 환자수) | 분포비(% · 기준 = 등록, 표시 전용) | 본인부담비(% · 디폴트 26.1% · 공단 지출: 참여 전 총 외래비 × (1−본인부담비), 참여 후 PB·타원비·비등록 × (1−본인부담비) · PF·성과 제외, v7.6.3~v7.6.7)` — 분포비 변경은 프리셋 버튼으로만
...
| **PF 통합 슬라이더 (B의 X%)** | **0~40%, step 0.5%** (v7.8.0 · 이전 0~20%) | — | **5%** (v7.2.2 · HCC 비례 자동) |
...
| **포괄관리 지표 C (ΔC)** | **0~+25%p, step 0.5%p** (v7.8.1 복귀 · v7.8.0에서 +75%p로 확장했다가 되돌림) | 0~25 | **0%p = L1 수준** (성과 0 기준점) |
...
| M (사업 참여 의원 수) | 프리셋 100·1,000·3,000·10,000·일만시 2,923 (v7.7.4) | ≥ 1 | **100** (1차년도 디폴트 · v7.1.1) |
...
**내보내기** (v7.7.7): `handleExport`가 수기 입력·계산 사용 열만 **6열** `환자군 | A | CR | C1(%) | RN | 본인부담비(%)`로 출력 (F 보정율은 v7.7.2부터 테이블 표시 전용, NT는 v7.7.7부터 테이블 미표시라 제외). 내보낸 파일을 그대로 업로드하면 테이블이 동일하게 복원(L = 1 − C1, 분포비 = RN 비율, M1 보존, PF는 B 변경 시 비율 보존). (v6.4 4열 템플릿·NHIS 엑셀도 업로드 호환 유지.)
...
pt_base·baseN_per_clinic·M_clinics·regDist(분포비 프리셋)·PF(F_g, PF 슬라이더 전용)는 엑셀로 이관 안 됨 (UI에서만 설정). v7.7.7부터 A·CR·C1·RN·본인부담비 5열만 내보내기·업로드 라운드트립 (NT는 baseline 데이터 필드로만 보존). B·PB·PF·P는 엑셀에 없고 업로드 시 A × CR 등으로 재산출(자가오염 방지).
...
5. **PF Track 배수 (v7.7.6)**: 등록환자 1인당 PF = PF × (A 0 / B 0.5 / C 1.0). 이전 "모든 Track에 F 가산" 원칙은 2026-09-05 사용자 결정으로 폐기. 공단 지출 PF 항도 같은 배수(공단 직접 지급 = 의원 수령액).
6. **본인부담 항 없음 (v7.6.1)**: 참여 후 수입·공단 지출에 본인부담(구 M1×30%, v7.6.0 PB×본인부담비)을 더하지 않는다. 등록환자 1인당 수입 = P.
7. **공단 지출의 본인부담비 (v7.6.3 · v7.6.5 · v7.6.6 · v7.6.7)**: 참여 전 `nhi0 = 총 외래비 × (1 − 본인부담비)`. 참여 후 = `(PB + D1_L2) × (1 − 본인부담비) × n_reg + PF × n_reg + C1 × (1 − 본인부담비) × n_unreg + 포괄관리성과` (PF·성과는 공단 직접 지급이라 전액 공단, 공단 성과 지출 = 의원 성과 수령액). 의원 수입 `P = PB + PF`에는 적용하지 않는다.
