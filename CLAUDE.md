...
2. **일차의료 기능보정 (F)** — B와 동일 구조 4 슬라이더 + NumBox
   - 액션 버튼 3개 + 초기화 (우상단):
     ...
     - **만원 맞춤** — B를 만원 단위 올림한 차액을 F에 배정 → P가 만원 단위로 정돈 (B가 이미 만원 배수인 군은 기존 F 유지)
...
9. **⚙️ 데이터 관리** (접힘 · ▼ 좌측) — 엑셀 업로드/내보내기, 파일럿 로드, **환자군별 상세 편집 테이블** (펼침 시 즉시 노출, 이중 접힘 없음)
   - 필드 순서 (산출 논리): `환자군 | 기준의료비 | 의원비중 | B=기준×비중 | F | P=B+F | L | M1 | N`
   - 편집 가능: 기준의료비, 의원비중(0~1 소수), F, L(0~1 소수), M1, N, B(슬라이더 override)
   - 자동 산출: 기준의료비·의원비중 변경 시 `B = round(ref × cr)` 재계산 후 state.P 동기화 (슬라이더로 이후 독립 조정 가능)
