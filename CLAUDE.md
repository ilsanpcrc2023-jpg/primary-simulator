...
**시뮬레이터 버전**: **v6.2.3** (모바일 UX · F 액션 버튼 재설계 · 편집 테이블 승격 · B 자동 산출)
...
2. **일차의료 기능보정 (F)** — B와 동일 구조 4 슬라이더 + NumBox
   - 액션 버튼 3개 + 초기화 (우상단):
     - **균등** — 1군 F 값을 2·3·4군에 복사
     - **차등** — 1군 F 값 기준 1:2:3:4 비율 (v, 2v, 3v, 4v)
     - **끝자리 보정** — B의 만원 이하 끝자리를 F로 이전 → P가 만원 단위로 정돈 (B가 이미 만원 배수인 군은 기존 F 유지)
     - **↩ 초기화** — INIT_F (차등 1·2·3·4만원) 복귀
3. **일차의료수가 (P = B + F)** — 항상 펼침
...
9. **⚙️ 데이터 관리** (접힘 · ▼ 좌측) — 엑셀 업로드/내보내기, 파일럿 로드, **환자군별 상세 편집 테이블** (펼침 시 즉시 노출, 이중 접힘 없음)
   - 필드 순서 (산출 논리): `환자군 | HCC 평균 | 의원비중 | B=HCC평균×비중 | F | P=B+F | L | M1 | N(이용·전체) | 등록(의원당)`
   - 편집 가능: HCC 평균(ref), 의원비중(cr, 0~1 소수), F, L(0~1 소수), M1, N, 등록(regDist), B(슬라이더 override)
   - 자동 산출: HCC 평균·의원비중 변경 시 `B = round(ref × cr)` 재계산 후 state.P 동기화 (슬라이더로 이후 독립 조정 가능)
   - 표시: 의원비중·L은 편집 테이블에서만 소수(`0.701`) 표시. KPI·차트는 % 유지
   - 등록 분포 프리셋: 부록/균등/건강편중/고위험편중 (테이블 우상단, 환자군 패널 카드에서 이동)
...
|---|---|---|---|
| B (환자군 기본수가) | 5만~200만원 | ≥ 0 | **INIT_BASE에서 자동 산출** (ref × cr, 약 28/42/73/105만원) |
...
F_g·regDist·pt_base는 엑셀로 이관 안 됨 (UI에서 설정).
...
- 버전 태그 이력: `v5.0` · `v6.0.0` · `v6.1.0` · `v6.2.0` · `v6.2.1` · `v6.2.2` · **`v6.2.3`**
