...
**시뮬레이터 버전**: **v7.0.2** (모바일 탭 라벨 SS → 성과배분)
  · **모바일 탭 라벨 한국어화** — `src/App.jsx` TABS 배열 세 번째 항목 `short: "💰 SS"` → `short: "💰 성과배분"`. 영문 약어 대신 한국어로 표시하여 모바일 사용자 직관성 개선. full 라벨("💰 성과 배분 (Shared Saving)")은 유지.
...
│                                # INIT_PF_PCT=10, INIT_PF_RULE="hcc" (v6.10.0), INIT_REG_DIST,
...
| **PF 통합 슬라이더 (B의 X%)** | **0~20%, step 0.5%** (v6.10.0) | — | **10%** (HCC 비례 자동) |
| PF 환자군별 (= F) | 0~60만원 (개별 미세 조정) | ≥ 0 (음수 불허) | **B의 10% = 28,083 / 30,020 / 52,358 / 74,532원** (v6.10.0 · HCC 비례 자동) |
...
- 의원당 등록환자 1,000명 (14.4%) · 비등록 5,960명 (85.6%) (regDist=[100,600,200,100])
...
- **PF 환자군별 = B의 10% (v6.10.0 · HCC 비례 자동)**: 28,083 / 30,020 / 52,358 / 74,532원 (분배 규칙 디폴트 hcc · 통합 슬라이더 10%)
- **L1 디폴트 = base.L 실측 자동 산출** (v6.9.5 · 파일럿: 0.7975 / 0.7934 / 0.7943 / 0.7722 · 가중평균 78.6%)
- **L2 디폴트 = L1 가중평균** (변화율 0%p 기준점, 성과급 0)
- **P = B(1−L1) + PF** 기본값(L1=base.L): 약 84,950 / 92,041 / 159,432 / 244,315원 (파일럿 실측 L · PF=B×10% 적용)
...
- **PF 카드** (resetF): INIT_F (= B × 10%, HCC 비례 자동) + pfRule = "hcc" 함께 복귀 (v6.10.0)
