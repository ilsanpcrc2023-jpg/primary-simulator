...
**시뮬레이터 버전**: **v6.8.1** (용어 재편 "타원이용비중 L2→포괄관리 지표 L2" · "성과급→포괄관리 성과가산" · 모드 토글 좌측 세그먼티드(남색/녹색, 2배 크기) · 첫 진입 의원 모드+수가 탭 · 의원 모드 대폭 단순화(정책 고정값·공식 아코디언·L1 개별표시·P=공단지급 부제 숨김))
