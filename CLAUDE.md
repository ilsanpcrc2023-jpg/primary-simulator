...
**시뮬레이터 버전**: **v7.8.1** (v7.5 데이터 + v7.6.x~v7.8.x 산식·UI 보완 · 최신: C 상한 +25%p 복귀, v7.8.0 PF 상한 40%, v7.7.9 테이블 캡션 문구, v7.7.8 풋노트 미표시, v7.7.7 NT 열 미표시·엑셀 제외, v7.7.6 PF Track 배수 A 0 / B 0.5 / C 1.0)
...
| 본인부담비 | 환자군별 본인부담비 (디폴트 **26.1%** · v7.6.4 · 테이블 편집 가능). **공단 지출에만 사용**: 참여 전 `nhi0 = C1 × N × (1 − 본인부담비)` (v7.6.3), 참여 후 PB·타원비 D1_L2·비등록 C1 × (1 − 본인부담비) (v7.6.5/v7.6.6). PF·포괄관리성과는 공단 직접 지급이라 100% (v7.6.7). 의원 수입에는 없음 (v7.6.1) | 0~1 |
