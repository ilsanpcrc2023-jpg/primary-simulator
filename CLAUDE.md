...
**시뮬레이터 버전**: **v6.4.1** (v6.4.0 기반 + UI 폴리시: P 카드 공단지급 보조표시·L↔P 박스 순서 교체·환자수 프리셋 3개·모바일 파이 차트 클리핑 수정)
...
3. **타원이용비중 (L) 변화율** — 슬림 박스 · 우측 NumBox + "↩ 초기화" (L이 P 카드의 공단지급 계산에 영향 → 입력값을 결과보다 먼저 배치)
4. **일차의료수가 (P = B + F)** — 항상 펼침. 환자군별 카드에 **P (indigo)** + dashed 분리선 + **공단지급 (rose)** 동일 크기로 병기. 하단에 수식·설명 박스: `공단지급 = B×(1−L) + F` (일차의료수가에서 타원이용비중만큼 차감 후 지급) / `의원 수입 = 공단지급 + 본인부담`
5. **KPI 2카드** (상시 표시)
   - 녹색 (**의원 수입 변화 분해**):
...
|---|---|---|---|
| B (환자군 기본수가) | 5만~200만원 | ≥ 0 | 22/30/52/74만원 (INIT_B 고정값 — 정책 슬라이더, 데이터와 무관) |
| F (일차의료 기능보정) | 0~60만원 | ≥ 0 | **차등 1·2·3·4만원** |
| regDist (환자군별 등록수) | 슬라이더 없음 | ≥ 0 | [100, 600, 200, 100] |
| LC | -30~0%p | 무제한 | -3%p |
| 참여 전·후 환자수 | 프리셋 3,000·5,000·7,000 (v6.4.1 단순화) | ≥ 1 | **3,000** |
...
- 버전 태그 이력: `v5.0` · `v6.0.0` · `v6.1.0` · `v6.2.0` · `v6.2.1` · `v6.2.2` · `v6.2.3` · `v6.3.0` · `v6.4.0` · **`v6.4.1`**
