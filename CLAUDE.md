...
**내보내기** (v7.7.0): `handleExport`가 상세 편집 테이블 전체 **16열** `환자군 | NT | RN | A | CR | B | C1(%) | PB | F(%) | PF | P | 분포비(%) | 본인부담비(%) | RR | M1 | L`로 출력. 내보낸 파일을 그대로 업로드하면 수기 입력 열(NT·RN·A·CR·C1·F·본인부담비)과 RR·M1·L이 복원되고, B·PB·P·분포비는 참고 열로 무시. (v6.4 4열 템플릿도 업로드 호환 유지.)
...
pt_base·baseN_per_clinic·M_clinics는 엑셀로 이관 안 됨 (UI에서만 설정). v7.7.0부터 regDist(RR)·F(F% 또는 PF)·본인부담비·NT·A·CR·C1은 내보내기·업로드 라운드트립. B는 엑셀에 참고 열로 쓰이지만 업로드 시엔 A × CR로 재산출(자가오염 방지).
