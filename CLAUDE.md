...
**시뮬레이터 버전**: **v7.2.3** (TCard 안내 문구 삭제 · v7.2.2 누적 + 본인부담 해석 혼동 차단)
  · **TCard "의원 수입 = 일차의료수가(공단지급) + 본인부담" 문구 삭제 (v7.2.3)** — 사용자 결정. 시뮬 산식 (공단지급 = P, 본인부담 = M1×30% 별도) vs 정책 시멘틱 (수가의 70% 공단·30% 본인부담) 사이 해석 충돌. v7.3.0에서 산식 정정 후 별도 안내 재검토.

**다음 작업 (v7.3.0)**: **모형 효과 = PF만 (시뮬 산식 정정)** — 사용자 OK 사인 받음. PF=0%여도 의원 수입 +1,312만원/의원당 발생하는 모순(B 베이스 NT vs M1 베이스 NC 비대칭) 정정. PB = M1×0.7 (공단지급분), 본인부담 양쪽 상쇄, 성과급 베이스 = M1로 정합. 정책 발표 시 "왜 PF 없이도 의원 수입이 늘어나나?"라는 시민단체·언론 비판 차단. 인계장: [docs/handoff_v7.3.0_pf-only-semantic-fix.md](docs/handoff_v7.3.0_pf-only-semantic-fix.md). 직전에 한 번 백지화된 안의 부활 — 화면 검증 후 사용자가 정책 안전성 우선 판단.
