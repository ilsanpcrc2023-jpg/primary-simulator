...
**시뮬레이터 버전**: **v7.4** (공단 지출 산식 정합 — 본인부담 양쪽 상쇄 + L2 격리, 의원 수입 = 공단 지출 절대값 일치)

  · **공단 지출 산식 정정 (v7.4)** — 데이터 사전 확인 결과 A·M1·RC·CO 모두 **총의료비**(공단지급 + 본인부담) 확정 ([docs/data_dictionary_v7.4.md](docs/data_dictionary_v7.4.md)). 시뮬 공단 지출 산식이 baseline·after 양쪽에 본인부담 30% 포함 + 등록환자 타원비를 L2 기반으로 계산해 PF 외 부산물 발생하던 문제 해소.
    - `nhi0 = C1 × N` → **`nhi0 = C1 × 0.70 × N`** (공단지급분만)
    - `nhi`의 등록환자 항 `(ab_reg + D1_L2) × n_reg` → **`(pay_gov_reg + D1_base_reg × 0.70) × n_reg`** (L2 격리, b.L 기반)
    - `nhi`의 비등록환자 항 `C1 × n_unreg` → **`C1 × 0.70 × n_unreg`** (공단지급분만)
    - L2 효과(포괄관리 지표 C 변화)는 별도 `performance.perf_blended` 모듈로 유지, KPI 카드에 "포괄관리성과" 분리 노출
    - 사용자 정책 의도: "환자 본인부담은 시범사업 전후 동일, 의원 수입·공단 지출 모두 PB·PF에 따른 변화만 노출"

  · **검증 결과 (v7.4 vs v7.3.0)**:
    - PF=5% (디폴트): 의원 수입 +3,281만원/년 = **공단 지출 +32.8억원/년** (= 의원당 +32,810만 × 100, 정확히 같은 절대값) [v7.3.0은 공단 +23.8억으로 차이 있었음]
    - PF=0%: 의원 수입 +103만원/년 = **공단 지출 +1.0억원/년** (성과급만 양쪽 동일) [v7.3.0은 공단 −8.0억으로 부호 불일치]
    - "왜 의원 수입·공단 지출이 늘어나는가?" → "**PF 가산만 (+ 포괄관리성과)**" 1:1 정합 정책 발표 가능
    - 단위 테스트 89/89 통과 (v7.3.0 86 + 신규 3: 공단 변화 = PF_g · PF=0 시 공단 변화 = 0 · 의원=공단 정합)
    - 인계장: [docs/handoff_v7.4_payer_alignment.md](docs/handoff_v7.4_payer_alignment.md)

  · **추가 데이터 불요 (v7.4)** — 본인부담률 차등(환자군별·타원 40~60% 등)은 본인부담 양쪽 상쇄로 변화 산식에서 사라지므로 현 시점 정확도 영향 미미. 30% 단일 가정 유지.

---

**시뮬레이터 버전 (v7.3.0 누적)**: 모형 효과 = PF만 · 시뮬 산식 정정 + 의료비 0원 제외 baseline

  · **의료비 0원 제외 baseline 갱신 (v7.3.0)** — 2025년 건강보험 이용자 전수자료에서 의료비 0원 환자 제외 후 NHIS-HCC v3.0 재구축 (전체 48,874,201명 · 참여의원 환자 12,411,152명, 이전 12,801,143명에서 -39만명). 환자군 평균 의료비 A는 실측값(HCC 예측 평균 H가 아님). 의원당 환자수: 4,379 → **4,246**. A·B·M1·L·CR·RR 4군 모두 갱신:
    - A = [405,180 · 767,703 · 1,355,881 · 5,589,034]원 (1·2군 +30~44%)
    - B = A × CR = [238,515 · 413,166 · 662,478 · 1,013,352]원
    - M1 = [86,457 · 125,057 · 189,058 · 328,901]원
    - L1 = [0.7165 · 0.7448 · 0.7605 · 0.7419]
    - RR = [201 · 198 · 294 · 307] (largest-remainder rounding, 합 1,000)
    - PF = B × 5% = [11,926 · 20,658 · 33,124 · 50,668]원 (PF 디폴트 5% 유지)
    - 새 엑셀: `docs/NHIS-HCC_Simulator_exc_zero_260518.xlsx`

  · **PB 정의 정정 (v7.3.0)** — `PB = B × (1−L1)` → **`PB = M1 × 0.70`** (공단지급분). 이전 PB는 NT(환자군별 전체 환자수) 베이스, M1은 NC(참여의원 환자수) 베이스라 코호트 불일치로 PB ≠ M1×0.7. 시뮬 본체에서 사용자 정책 의도 정합: "환자 본인부담 변화 없음, 의원 받는 돈 변화 없음, **변화는 PF만**". PB(M1×0.7) + 본인부담(M1×0.3) = M1, 즉 baseline FFS(M1) 대비 delta = PF만. A·B·CR·NT·B×C1은 reference로 강등 (상세 편집 테이블에 reference로 표시).

  · **성과급 베이스 B → M1 (v7.3.0)** — 성과급 = Σ max(0, L1_g − L2) × **M1** × n_reg × TrackMul (이전 B 베이스). 시뮬 핵심 산식(M1 베이스)과 정합. 의미: 등록환자가 등록의원 외래로 더 집중(L2 < L1, C2 > C1)할 때 그 절감분을 의원에 환원. UI에는 v7.0의 "포괄관리성과" 라벨 그대로.

  · **modelEffect = PF 가산만 (v7.3.0)** — `modelEffect = Σ n_reg × (ab_reg − M1) = Σ n_reg × PF`. **PF=0%이면 modelEffect=0** (구조 전환만으로는 의원 수입 변동 없음). 정책 발표 시 "왜 의원 수입이 늘어나는가?" → "PF 가산만"으로 명확하게 답변. 시민단체·언론 "PF 없이도 의원 수입 자동 증가" 비판 차단.

  · **검증 (v7.3.0)** — 디폴트 시나리오(M=100, 의원당 등록 1,000명, PF=5%, L2=L1avg):
    - PF=0% → 의원 수입 변화 = **+103만원/년** (modelEffect=0, 성과급만 — L1>L1avg인 환자군 효과)
    - PF=5% → 의원 수입 변화 = **+3,281만원/년** (PF +3,178만원 + 성과급 +103만원)
    - 정책 모드 PB 카드: 1군 60,520 · 2군 87,540 · 3군 132,341 · 4군 230,231원 = M1×0.7 ✓
    - 일차의료수가 P = PB + PF: 3군 165,465 · 4군 280,899원 ✓
    - 단위 테스트 86/86 통과 (회귀 방지 신규: PF=0 → delta=0 · PF=5% → delta=PF_g · pay_gov = M1×0.7 + F)
    - 인계장: [docs/handoff_v7.3.0_pf-only-semantic-fix.md](docs/handoff_v7.3.0_pf-only-semantic-fix.md)

  · **TCard 안내 문구 삭제 (v7.2.3 누적)** — 사용자 결정. 시뮬 산식 (공단지급 = P, 본인부담 = M1×30% 별도) vs 정책 시멘틱 (수가의 70% 공단·30% 본인부담) 사이 해석 충돌. v7.3.0에서 산식 정정으로 PB = M1×0.7 정합 회복 — 별도 안내는 재검토 보류.
