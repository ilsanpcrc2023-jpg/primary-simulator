...
**시뮬레이터 버전**: **v6.9.0** (UI 재구성 — 모드별 차등화 강화 · v6.8.3 누적)
  ...
  · **[D] Shared Saving 의원 모드 차등화** — 상단에 "🏥 우리 의원 예상 연간 성과배분" Hero 박스 신규 (산출 공식: 절감배분액 × Track 지급률). 항목별 절감·의료비 절감·배분 비율 카드는 fieldset disabled로 읽기 전용 처리. 정책 모드는 변경 없음.
