...
**시뮬레이터 버전**: **v6.7.0** (L1·L2 분리 — 선지급 vs 사후 성과급 · no-downside · Track 비례)
...
| **α** | **공유율 — L2 절감분 중 의원 환원 비율 (no-downside)** | 0~1 (기본 0.5) |
...
- **사후 정산층 (performance)** — L2는 사업 중 관측된 실측 타원이용. L2가 L1보다 낮으면 절감분의 α×TrackMul 비율을 의원에 성과급으로 환원. L2 > L1이어도 환수 없음 (no-downside, 시범사업 수용성).
- **Track 배수**: A(FFS)=0 / B(혼합)=0.5 / C(환자군)=1.0. 선형 보간(hccPct/100).
- **Shared Saving과 분리**: SS는 입원·응급·요양병원 절감(간접 관리), 성과급 L2는 외래 집중도(직접 행위). 재원 분리, 2년차부터 동일 분기에 패키지 지급.
...
| — | — | — | **α (공유율), 성과급 = max(0,L1−L2)×B×n_reg×α×TrackMul** |

코드 내부 변수명 `state.P`, `state.F_g`는 하위 호환을 위해 유지 (값은 각각 B·F). UI 라벨만 새 기호로 표시. `state.L1` (배열)·`state.L2` (스칼라, null=L1 가중평균)·`state.alpha`는 v6.7 신규. `state.LC`는 제거됨.
...
- α 기본 0.5, TrackMul: A=0 / B=0.5 / C=1.0
