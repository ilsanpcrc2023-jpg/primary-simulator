...
describe('v7.5.1 ratiosFromBase / regDistFromRatios', () => {
  ...
  it('v7.5.5: refRatiosFromBase = RN 기준 (N_i / ΣN) — NT는 참고 표시일 뿐 기준 분포비에 쓰이지 않음', () => {
    ...
    expect(sumN).toBe(11911125);   // v7.9.0 baseline 260909 (이전 exc_zero 12,411,152)
    rn.forEach((r, i) => expect(r).toBeCloseTo(INIT_BASE[i].N / sumN, 12));
    expect(rn.map(r => (r * 100).toFixed(2))).toEqual(['19.24', '22.97', '28.01', '29.79']);
  ...
  it('regDistFromRatios: INIT_BASE(v7.9.0 baseline 260909) RN 기준에 적용하면 INIT_REG_DIST와 일치 (등록 분포비 디폴트 = 기준 분포비)', () => {
  ...
  it('v7.5.3/v7.5.5: 디폴트 등록 분포비(%)는 기준 분포비(%, RN 기준)와 소수점 2자리까지 동일', () => {
    ...
    // v7.9.0 baseline 260909 RN 기준값: 19.24 / 22.97 / 28.01 / 29.79 % (이전 exc_zero 20.16 / 19.77 / 29.38 / 30.68)
    expect(INIT_REG_DIST.map(rr => (rr / 10).toFixed(2))).toEqual(['19.24', '22.97', '28.01', '29.79']);
  });
...
// v7.6.2: 본인부담비 참고값 상수 복원 → v7.6.3~ 공단 지출 산식 사용
// v7.9.0: INIT_COPAY_RATES는 official_baseline.json의 copay(군별)에서 로드, COPAY_RATE는 fallback 상수
describe('v7.6.4 / v7.9.0 COPAY_RATE / INIT_COPAY_RATES', () => {
  it('COPAY_RATE = 0.261 (fallback), INIT_COPAY_RATES = baseline 260909 군별 본인부담비', () => {
    expect(COPAY_RATE).toBe(0.261);
    expect(INIT_COPAY_RATES).toHaveLength(4);
    expect(INIT_COPAY_RATES).toEqual([0.280506, 0.2856, 0.264511, 0.205334]);
    INIT_COPAY_RATES.forEach(v => { expect(v).toBeGreaterThan(0); expect(v).toBeLessThan(1); });
  });
});
