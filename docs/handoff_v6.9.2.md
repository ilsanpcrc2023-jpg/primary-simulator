...
## 3-bis. 1차 구현 후 사용자 피드백 반영 (커밋 7bc1c12 → 41d88d3 → 0c9866d)

1차 구현(19690ee) 후 사용자 피드백 3차례에 걸쳐 다음 보강:

### 3-bis.1 위치 이동 + M_clinics 미러 (7bc1c12)

- **마운트 위치**: B+F 통합 박스 직후·L1 박스 직전. 새 순서 B → F → ⚖️ 균형추 → L1 → P → L2. "✓ F 슬라이더에 적용" 결과를 위쪽 F 박스에서 즉시 확인 (피드백 흐름 단절 해소).
- **M_clinics 미러 프리셋 추가** (이후 0c9866d에서 다시 NumBox로 교체).
- TabSimulation의 `policyVariables`를 `policyBF`(B+F 통합)와 `policyL1`(L1)로 분리.

### 3-bis.2 totalN 자동 동기화 (41d88d3)

reducer SET 핸들러에서 `action.key === "M_clinics"` 시 `totalN = (totalN/M) × newM` 자동 갱신. 이전에는 M=1,000으로 늘리면 totalN=300,000 clamp으로 의원당 등록 179명까지만 채워졌으나, 이제 totalN=3,000,000 자동 확장으로 의원당 등록 1,000명 정합. baseN_per_clinic은 "참여 전 기준선"(패널 변화 효과 분해용)이라 변경 안 함.

### 3-bis.3 좌측 카드 + 등록환자 NumBox (0c9866d, 사용자 피드백 3건)

| 영역 | 변경 |
|---|---|
| 좌측 윈윈 카드 | "포괄관리 성과가산 잠재" → **"공단 외래 지출 변화"** (현재 시뮬 vs 균형추 적용 후 2칸 비교). 균형추는 F를 +ΔF만큼 늘리는 도구이므로 공단 지출도 +ΔF 증가. 정책 trade-off가 한눈에 — 좌측에 "절감폭 축소" / 우측에 "의원 수입 강화" 동시 노출. |
| 헤더 사업 규모 | M_clinics 프리셋 4버튼 → **등록환자 규모 NumBox** (직접 입력). 정책 의사결정 1차 변수를 "의원 N개"가 아닌 "등록 N명" 절대 인원으로 노출. M_clinics는 derive (등록 / 의원당 등록). |
| 프리셋 버튼 | M_PRESETS 4개 완전 제거. NumBox 단일 입력 경로. |

도메인 매핑: `nhiBaseline = T.nhi0` / `nhiNow = T.nhi` / `nhiAfterBalance = T.nhi + deltaTotal`. `additionalPerf_5pp_*` 변수는 좌측 카드 대체로 불필요 — 제거.

### 3-bis 검증

| 시나리오 | 결과 |
|---|---|
| M=100 (디폴트) | 등록환자 100,000명 / 분모 1,975.7억 / 좌측: -88.7억 → -29.4억 (균형추 적용 시 절감폭 축소) |
| M=1,000 | 등록환자 1,000,000명 / 분모 1.98조 / totalN 자동 확장 3M / 의원당 평균 1,000명 정합 |
| 환자군 패널 ↔ 균형추 헤더 양방향 동기화 | M_clinics 단일 진실 소스 |

51/51 테스트 통과 / 빌드 성공 / 콘솔 에러 0건.
